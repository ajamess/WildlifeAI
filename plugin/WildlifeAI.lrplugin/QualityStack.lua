...
function M.stackByScene(photos)
  ...
  for _,p in ipairs(photos) do
    local sc = p:getPropertyForPlugin(_PLUGIN, 'wai_sceneCount') or 0
  ...
  catalog:withWriteAccessDo('WildlifeAI Stack', function()
    for _,arr in pairs(groups) do
      table.sort(arr, function(a,b)
        return (a:getPropertyForPlugin(_PLUGIN,'wai_quality') or 0) >
               (b:getPropertyForPlugin(_PLUGIN,'wai_quality') or 0)
      end)
      local top = arr[1]
      for i=2,#arr do
        catalog:createPhotoStack(top, arr[i])
      end
    end
  end)
