...
return function()
  ...
  table.sort(photos, function(a,b)
    local qa = a:getPropertyForPlugin(_PLUGIN,'wai_quality') or 0
    local qb = b:getPropertyForPlugin(_PLUGIN,'wai_quality') or 0
    if qa == qb then
      local ca = a:getPropertyForPlugin(_PLUGIN,'wai_colorConfidence') or 0
      local cb = b:getPropertyForPlugin(_PLUGIN,'wai_colorConfidence') or 0
      return ca > cb
    end
    return qa > qb
  end)
